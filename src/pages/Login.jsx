...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const isNurseOrLab = ('director' === 'nurse' && (response.user.role === 'nurse' || response.user.role === 'hamshira')) || ('director' === 'lab' && (response.user.role === 'lab' || response.user.role === 'laborant'));
      
      if (response.user.role !== 'admin' && response.user.role !== 'director' && !isNurseOrLab) {
        setError("Bu panel faqat Direktor uchun! Siz " + response.user.role + " rolidagilar uchun boshqa panelga kirishingiz kerak.");
        setLoading(false);
        return;
      }
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-600 via-blue-500 to-cyan-500 flex items-center justify-center p-4">
      <div className="relative w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-white/80 mt-2 font-medium">Direktor paneli</p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-8">
          ...
          <div className="mt-6 pt-5 border-t border-gray-100">
            ...
            >
              <div>
                <p className="text-xs text-gray-500 font-medium">Direktor uchun:</p>
                <p className="text-sm font-semibold text-gray-700">Login: <span className="font-mono text-blue-600">director</span></p>
              </div>
              <div className="text-right">
                <span className="text-xs text-gray-500 block">Parol:</span>
                <span className="text-xs font-mono font-bold bg-gray-200 text-gray-700 px-2 py-1 rounded">director123</span>
              </div>
            </div>
